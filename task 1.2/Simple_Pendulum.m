...
function dy = pendulum_dynamics(y, m, L, g, u)
  ...
  dy(1,1) = y(2);                                  
  dy(2,1) = -g*(sin_theta/L) + (1/(m*L*L))*u;    
endfunction
...
function [t,y] = pole_place_pendulum(m, g, L, y_setpoint, y0)
  [A,B] = pendulum_AB_matrix(m,g,L);  
  #mat=zeros(2,1);  ## Initialize A and B matrix
...
function [t,y] = lqr_pendulum(m, g, L, y_setpoint, y0)
  ...
  [mat,h,l]=lqr(A,B,Q,R);
  K = mat;    
  #disp(K);  ## Calculate K matrix from A,B,Q,R matrices
...
function simple_pendulum_main()
  ...
  #disp([t,y]); ## Test Simple Pendulum with Pole Placement Controller
  #[t,y] = lqr_pendulum(m,g,L, y_setpoint, y0);   
  #disp([t,y]);  ## Test Simple Pendulum with LQR Controller
